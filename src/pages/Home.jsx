      ...
      >
        ...
        <section id="about" className='pt-[50px] pb-[50px] bg-[--bg]'>
          <Container>
            <div className="w-full flex justify-end items-end  md:mb-4 md:items-center md:justify-start">
              <h2 className='text-[35px] lg:text-[25px] font-bold text-[#3498db] '>{t("iron")} <span className='text-[--text] font-[200]'>{t("guc")}</span></h2>
            </div>
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[#3498db] uppercase  '>{t("metalcon")} <span className='text-[--text] '>{t("haqqinda")}</span></h2>
            </div>
            <Row className='items-center '>
              ...
              <Col lg={6} className='flex items-center  flex-col'>
                <div className="w-full text-justify line-clamp-5 lg:line-clamp-3 mb-4 text-[--text]" dangerouslySetInnerHTML={{ __html: options?.text_az && ml(options?.text_az,options?.text_ru,options?.text_en)  }}></div>
        ...
        <section id="spesfics" className='pt-[50px] pb-[50px] bg-[--bg2]'>
          <Container>
            ...
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[--text] uppercase'>{t("char")} </h2>
            </div>
            <Row className=''>
              {
                  ...
                  <Col lg={6} md={12} xs={12} key={i} className='p-[30px] lg:p-[15px] flex items-center spesfics '>
                    <div className="flex p-[30px] md:p-[15px] md:flex-col md:items-center ">
                      ...
                      <div className="flex flex-col pl-[30px] md:items-center md:pl-[0]">
                        <h2 className='text-[25px] font-[600] text-[--text] '>{ml(cur?.name_az,cur?.name_ru,cur?.name_en)}</h2>
                        <div className='mt-[20px] text-justify text-[--text]' dangerouslySetInnerHTML={{ __html: cur?.text_az && ml(cur?.text_az,cur?.text_ru,cur?.text_en) }}></div>
                      </div>
                    </div>
                  </Col>
                ))
        ...
        <section id="projects" className='pt-[50px] pb-[50px] bg-[--bg]'>
          <Container>
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[--text] uppercase'>{t("projects")} </h2>
            </div>
            <Row className=''>
              {
           ...
           <Link to='/projects'  className="bg-[#eee] hover:bg-[#3498db] hover:text-[#fff] transition max-w-max  border-none capitalize outline-none shadow1  mr-0 mb-[10px] rounded-[4px] pt-[5px] pb-[5px] pl-[30px] pr-[30px] text-[#000] text-[17px] ">
               {t("loadmore")}
                </Link>
           </div>
          </Container>
